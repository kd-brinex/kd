<?php
$responseConfig = require('converter.php');
/**
 * @author: Eugene Brx
                    ...
                    'methods' => [                                              // связка методов api провайдера с нами, определяется как массив 'вызываемое_нами_имя_метода' => ['name' => 'имя_метода_провайдера', 'uri_index' => 'индекс_uri_где_описывается_метод_провайдера']
                        'findDetails' => [
                              ...
                              'params' => [
                                  ...
                                  'substFilter' => 'None',                      // фильтр по типу деталей (обязательно). Параметры: None - не фильтровать; FilterOriginalAndReplacements - только искомый номер, новый номер и замены искомого номера; FilterOriginalAndAnalogs - только искомый номер и аналоги.
                                  'deliveryRegionType' => '',                   // тип доставки PRI; ALT (обязательно, по умолчанию надо указывать PRI)
                                  'detailNum' => '',                             // номер детали
                              ]
                        ],
                        'toBasket' => [
                            'name' => 'InsertToBasket3',
                            'uri_index' => 0,
                            'params' => [
                                'login' => '',
                                'password' => '',
                                                                                // Массив ePrices[EPrice[],EPrice[], ... , EPrice[]] требует провайдер
                                'ePrices' => [
                                    'EPrice' => [
                                        'Num' => '1',                           // номер по порядку (произвольный номер)
                                        'MLogo' => '',                          // лого изготовителя (MakeLogo)
                                        'DNum' => '',                           // уникальный номер детали (DetailNum)
                                        'Name' => '',                           // наименование детали русское (DetailNameRus все большие)
                                        'Quan' => '1',                          // количество деталей (max Quantity)
                                        'Price' => '',                          // цена рублевая (пример: 0000.0000)
                                        'PLogo' => '',                          // лого прайса детали (PriceLogo)
                                        'DLogo' => '',                          // лого доставки (DestinationLogo)
                                        'DeliveryRegionType' => '',             // тип доставки по региону. Если этот элемент не задан, то сохраняется значение PRI
                                        'Ref' => '',                            // поле reference (это и есть комментарий)
                                        'Com' => '',                            // поле комментария. Также в этом поле можно задавать разрешенные признаки детали. Вводить их в любом порядке, через пробел. Например BRAND ONLY
                    ...
                    'methods' => [
                        'findDetails' => [
                            ...
                            'params' => [
                                ...
                                'StockOnly' => '',                              // Только наличие на складе компании. False - нет, True - да
                    ...
                    'methods' => [
                        'findDetails' => [
                            ...
                            'params' => [
                                ...
                                'findSubstitutes' => false,                     // boolean Флаг для поиска с заменами и аналогами или без них.
                                'store' => true,                                // boolean Флаг для поиска только в наличии склада «ПартКом».
                    ...
                    'methods' => [
                        'findDetails' => [
                            ...
                            'params' => [
                                ...
                                'act' => 'price_by_nr_firm',                                    // = price_by_nr_firm" - Название функции
